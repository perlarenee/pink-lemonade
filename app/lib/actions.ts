...
/*export async function createRefreshment(formData:FormData){
    const rawFormData = {
        ...
        format: formData.get('formats'),
        length: formData.get('length'),
        status: formData.get('status')
    }
    console.log('tag data',rawFormData); 


    const { contributorId, title, content, image_url, tags, formats, length, status} = CreateRefreshment.parse({
        contributorId: formData.get('contributorId'),
        title: formData.get('title'),
        content: formData.get('content'),
        image_url: formData.get('image_url'),
    ...
    VALUES (${contributorId}, ${title}, ${content}, ${image_url}, ${tags}, ${formats}, ${length}, ${status}, ${date})
    `
...
const CreateRefreshment = FormSchema.omit({ id: true, date: true });
const UpdateRefreshment = FormSchema.omit({id:true, date: true });
